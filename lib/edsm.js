...
// Declare user agent to avoid CloudFlare blocking requests.
const requestOptions = { headers: { 'User-Agent': `Ardent/${Package.version} (ardent-insight.com)` } }

class EDSM {
  static async getSystemStatus (systemId64) {
    const resSystem = await axios.get(`${baseUrl}api-v1/system?systemId64=${encodeURIComponent(systemId64)}&showInformation=1&showCoordinates=1&showId=1&showPermit=1`, requestOptions)
  ...
  static async getSystemBodies (systemId64) {
    const res = await axios.get(`${baseUrl}api-system-v1/bodies?systemId64=${encodeURIComponent(systemId64)}`, requestOptions)
  ...
  static async getSystemStations (systemId64) {
    const res = await axios.get(`${baseUrl}api-system-v1/stations?systemId64=${encodeURIComponent(systemId64)}`, requestOptions)
