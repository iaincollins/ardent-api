...
const os = require('os')
const { SYSTEMS_DB_REF, STATIONS_DB_REF, TRADE_DB_REF, DB_REF } = require('../consts')
...
// Provide read only, non-blocking methods for handling queries using workers

const systemsDbAsync = {
  all: (sql, params = {}) => asyncQuery(SYSTEMS_DB_REF, sql, params),
  get: async (sql, params = {}) => {
    return (await asyncQuery(SYSTEMS_DB_REF, sql, params))?.[0] ?? null
  }
}

const stationsDbAsync = {
  all: (sql, params = {}) => asyncQuery(STATIONS_DB_REF, sql, params),
  get: async (sql, params = {}) => {
    return (await asyncQuery(STATIONS_DB_REF, sql, params))?.[0] ?? null
  }
}

const tradeDbAsync = {
  all: (sql, params = {}) => asyncQuery(TRADE_DB_REF, sql, params),
  get: async (sql, params = {}) => {
    return (await asyncQuery(TRADE_DB_REF, sql, params))?.[0] ?? null
  }
}

const dbAsync = {
  all: (sql, params = {}) => asyncQuery(DB_REF, sql, params),
  get: async (sql, params = {}) => {
    return (await asyncQuery(DB_REF, sql, params))?.[0] ?? null
  }
}
